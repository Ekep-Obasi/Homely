...
import { Menubar, MenubarContent, MenubarItem, MenubarMenu, MenubarTrigger } from '../ui/menu-bar'
...
export function KeyBoard() {
  const { showPicker, setShowPicker, message, setMessage }: any = React.useContext(KeyBoardContext)

  return (
    <div className="absolute min-h-[65px] bg-primary w-full bottom-0 text-secondary flex items-center gap-x-2 px-1 z-50">
      <Icons.Smile className="hover:cursor-pointer" onClick={() => setShowPicker((prev: any) => !prev)} />
      <AddOptions />
      <textarea
        value={message}

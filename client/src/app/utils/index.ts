...
export function getUserEntries(user: User | null) {
  return (
    user &&
    Object.keys(user)
      .filter((key) => user[key as UserKeyType] !== '')
      .reduce((acc, curr) => ({ ...acc, ...{ [curr]: user[curr as UserKeyType] } }), {})
  )
}

export function jsonToFormData<T extends object>(obj: T, formData: FormData) {
  Object.keys(obj)
    .filter((key) => typeof obj[key as keyof T] === 'string' || obj[key as keyof T] instanceof Blob)
    .forEach((key) => {

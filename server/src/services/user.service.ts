...
import { GenerateSalt, HashPassord, SignAuthToken, ValidatePassword } from '../utils'
...
export async function CreateUser(req: Request, res: Response) {
  const { user_name, email, password } = <ICreateUserTypes>req.body
  ...
  if (user) {
    ...
    try {
      ...
      if (receiver && receiver._id == sender._id) {
        ...
        return res.send({
          message: 'Reciever does not exist || receiver is trying to post about his profile',
        })
      }
    } catch (err) {
      return res.send({
...
export async function RecoverPassword(req: Request, res: Response) {
  ...
  try {
    ...
    ejs
      .renderFile(path.join(__dirname, '../', 'views/password-recovery-email.ejs'), {

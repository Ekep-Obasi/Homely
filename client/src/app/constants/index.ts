import { OptionsList } from '../components/dropdown'
import { listingCategoryTypes } from '../types/listing'

export const houseQuality = ['Minimal', 'Classic', 'Modern'] as const
export const houseType = ['Room', 'Studio', 'Apartements'] as const
export const MAX_FILE_SIZE = 5000
export const ACCEPTED_IMAGE_TYPES = ['image/jpeg', 'image/jpg', 'image/png']
export const USER_STORAGE_KEY = 'user-data'
export const TOKEN_STORAGE_KEY = 'token'
export const LISTING_STORAGE_KEY = 'current-listing'

export const API_KEY = process.env.NEXT_PUBLIC_API_KEY
export const AUTH_DOMAIN = process.env.NEXT_PUBLIC_AUTH_DOMAIN
export const PROJECT_ID = process.env.NEXT_PUBLIC_PROJECT_ID
export const STORAGE_BUCKET = process.env.NEXT_PUBLIC_STORAGE_BUCKET
export const MESSAGING_SENDER_ID = process.env.NEXT_PUBLIC_MESSAGING_SENDER_ID
export const APP_ID = process.env.NEXT_PUBLIC_APP_ID
export const MAP_BOX_API_TOKEN = process.env.NEXT_PIBLIC_MAP_BOX_API_TOKEN
export const API_BASE_URL = process.env.NEXT_PUBLIC_BACKEND_BASE_URL

export const categories: listingCategoryTypes[] = [
  { label: 'All Categories', value: '' },
  { label: 'Modern', value: 'modern' },
  { label: 'Classic', value: 'classic' },
  { label: 'Minimalist', value: 'minimalist' },
]

export const qualityOptionList: OptionsList[] = [
  { label: 'All', value: '' },
  { label: 'Rooms', value: 'room' },
  { label: 'Studios', value: 'studio' },
  { label: 'Apartements', value: 'apartement' },
]
